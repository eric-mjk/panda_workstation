...
def rotate_clock(clock: int, offset_steps: int) -> int:
    """Rotate clock index by offset_steps. +3 means 12->3, 3->6, 6->9, 9->12."""
    if not (1 <= clock <= 12):
        raise ValueError(f"clock must be in [1, 12], got {clock}")
    return ((clock - 1 + offset_steps) % 12) + 1


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--clock", type=int, required=True, help="Input clock direction in our/camera convention: 1..12")
    parser.add_argument(
        "--clock-offset-steps",
        type=int,
        default=3,
        help="Clock offset from our/camera convention to world convention. Default +3: 12->3, 3->6, 6->9, 9->12.",
    )
    ...
    world_clock = rotate_clock(args.clock, args.clock_offset_steps)
    unit = clock_to_unit(world_clock)
    disp = [
    ...
    payload = {
        ...
        "clock_offset_steps": args.clock_offset_steps,
        "mapping": "manual_clock_plus_offset_to_world",
    ...
    print(f"Input clock         : {args.clock}  (our/camera convention)")
    print(f"Clock offset steps  : {args.clock_offset_steps:+d}")
    print(f"World clock used    : {world_clock}")
